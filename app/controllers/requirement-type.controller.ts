...
export let byApplication = (req: {applicationId: number}) => {
  return new Observable(observer => {
    const sql = `SELECT LO.id_object, name_object, color, code_object FROM light_object LO LEFT JOIN light_app_object_attribute LAOA ON LO.id_object=LAOA.id_object WHERE LAOA.id_app=${req.applicationId} and LAOA.active=1 group by LO.id_object order by LO.id_object;`;
