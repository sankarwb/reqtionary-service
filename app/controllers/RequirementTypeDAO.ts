...
import * as	attributeDAO from './AttributeDAO';
import * as	artifactDAO from './ArtifactDAO';
import * as	projectDAO from './ProjectDAO';


export const  reqtypeDAO = function (router: any) {
	router.post('/updateRequirementType',(req: any,res: any) => {
		...
		function callback(err: any, rows: any, fields: any) {
			...
			if(err){
				...
				if(req.body.attributes.length > 0)
					req.body.attributes.forEach((item: any) => promiseAll.push(attributeDAO.updateObjectAttributes(req.body.orgId,(req.body.id === 0)?rowData[0]['@out_status_two']:req.body.id,item,req.body.modifiedByID)));
				if(req.body.values.length > 0)
					req.body.values.forEach((item: any) => promiseAll.push(attributeDAO.updateAttributeValues(req.body.orgId,(req.body.id === 0)?rowData[0]['@out_status_two']:req.body.statusAttr.id,item,req.body.modifiedByID)));
	...
	router.post('/getObjectDetails',(req: any,res: any) => {
	...
	router.post('/getAppObjectsArtifactsAttributes',function(req: any,res: any){
	...
	router.post('/getObjectArtifacts',function(req: any,res: any){
		var columns = [], 
		...
		function callback(err: any, rows: any, fields: any) {
			...
			if(err){
				...
				var row = rows[0];
				row.forEach(function(rowData: any, index: any) {
					var artifactVO: any = {};
	...
	router.post('/getAssociateObjects',function(req: any,res: any){
		getAssociatedRequirementTypes(req).then(result => res.json(result), err => res.json({error:err}));
	});

	//Objects of an organizations
	router.post('/getRequirementTypes',function(req: any,res: any){
	...
	router.post('/getAppObjectAttributes',function(req: any,res: any){
				...
				rows.forEach(function(rowData: any, index: any) {
					if(objectVO.id === rowData['id_object']) {
						...
						if(objectVO.attributes && objectVO.attributes.length > 0) {
							let nullAttrIdx =  -1;
							objectVO.attributes.some((attr: any,idx: any)=>{
								if(attr.id === null) {
									nullAttrIdx = idx;
									return true;
								}
...
function getAssociatedRequirementTypes(req: any){
...
function getAppRequirementTypes(req: any){
	return new Promise(function(resolve,reject){
		var columns = ['id_object', 'name_object', 'code_object', 'color', 'seqnum_object'],
		qry = "";
...
function getObjectDetails(req: any) {
	return new Promise(function(resolve,reject){
		...
		function callback(err: any, rows: any, fields: any) {
			var response: any = [],objectVO: any = {},attributeVO: any = {},attributes: any = [];
			if(err){
...
export const updateAppObjAttributes = function(req: any) {
	return new Promise((resolve, reject) => {
		if( req.body.types && req.body.types.length > 0) {
			let promiseAll: any = [];
			req.body.types.forEach( (item: any) => {
...
function promiseAppObjAttr(orgId: any, appId: any, type: any, modifiedByID: any) {
	return new Promise((resolve,reject) => {
...
function getRequirementTypes(req: any) {
	return new Promise(function(resolve,reject){
