		...
		function callback(err: any, rows: any, fields: any) {
			if(err){
				...
				RoleDAO.updateBulkRoleEmp( req, (req.body.id === 0)?row[0]['@out_status_two']:req.body.id, 'appGrpId' )
						.then( (resp: any) => resolve({success:`${req.body.name} ${req.body.id === 0? 'created':'updated'} successfully`}) );
			} else {
