...
export const employeeDAO = function(router: any) {
	router.post('/getEmployees',(req: any,res: any) => {
		getEmployees(req).then(result => res.json(result),error => res.json(error));
	});

	router.post('/updateEmployee',(req: any,res: any) => {
		updateEmployee(req).then(results => res.json(results),error => res.json(error));
	});

	router.post('/verifyLogin',(req: any,res: any) => {
...
function verifyUserLogin(req: any){
	return new Promise((resolve,reject) => {
		...
		function callback(err: any, rows: any, fields: any) {
			...
			if(err){
				...
				var employeeVO: any = {},rowData = rows[0];
